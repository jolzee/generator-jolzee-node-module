...
module.exports = class extends Generator {
  ...
  _getNPMConfig() {
    const author =
      exec(`npm config get init.author.name`, { encoding: `utf-8` }) || ``;
    const email =
      exec(`npm config get init.author.email`, { encoding: `utf-8` }) || ``;
    return { author, email };
  }
  ...
  writing() {
    const files = [`example/script.js`, `src/index.js`, `__tests__/test.js`];

    files.forEach(f => this._copyFile(f));

    const eslint = [`.eslintignore`, `.eslintrc`];
  ...
  install() {
    ...
    this._spawn(
      `git remote add origin https://github.com/${github}/${name}.git`
    );
    ...
    spawn(`git`, [`commit`, `-m`, `${emoji}initial commit`], {
      stdio: `inherit`
    });
